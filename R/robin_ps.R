#' Post stratification based model
#' @description
#' Strata are constructed by the unique levels of treatment probability assignment
#'
...
#'   alpha = 0.05
#' )
robin_ps <- function(formula, data, treatment, prob_mat, treatments_for_compare,
  ...
  pc <- predict_counterfactual(
    fit.j = fit.j, fit.k = fit.k, treatment = treatment,
    treatments_for_compare = treatments_for_compare,
    prob_mat = prob_mat, data = data, stabilize = stabilize, method="ps"
  )
